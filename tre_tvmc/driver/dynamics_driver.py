...
class QDynamics(AbstractVariationalDriver):
    def __init__(
        ...
        *,
        ...
        self.target_state = copy_variational_state(
            variational_state,
            n_hot=1,
            copy_samples=True,
        ...
        if (
            n_samples_obs is None
            or isinstance(variational_state, nk.vqs.FullSumState)
            or n_samples_obs == variational_state.n_samples
        ):
            self._obs_state = None
        else:
            self._obs_state = copy_variational_state(
                variational_state,
                n_hot=0,
                variables=variational_state.variables,
                n_discard_per_chain=16,
            )  # increase discard per chain to burn in
        ...
        self._propagator_and_dagger = (
            None  # will store the actual propagator for each projection
        )
    ...
    def _project(self, n_iter, U=None, U_dagger=None, callback=None, **kwargs):
        with timing.timed_scope(name="dynamics_step_projection"):
            ...
            if self.random_noise is not None:
                ...
                self.target_state.parameters = add_noise_to_param_dict(
                    key, self.target_state.parameters, stddev=self.random_noise
                )
            ...
            if self.method in ("fidelity", "infidelity", "fid", "infid"):
                from .infidelity_psi import (
                    InfidelityOptimizer as InfidelityOptimizerImp,
                )
            ...
            if self.keep_best:
                ...
                if best_params is None:
                    if kwargs.get("show_progress", False):
                        print_mpi(
                            f"Trying to take best params at step {best_step} with loss {best_loss}, but found None params",
                            flush=True,
                        )
                    ...
                    if kwargs.get("show_progress", False):
                        print_mpi(
                            f"Found best model at step {best_step} with loss {best_loss}",
                            flush=True,
                        )
    ...
    def _advance_step(
        ...
        for op_nr, (k, (op, op_dag, op_dt)) in enumerate(
            self.tevo_generator.taylor_root_expansion(self.t, dt=self.dt).items()
        ):
            ...
            out_inner = nk.logging.JsonLog(
                out_inner, save_params=False
            )  # this will use less cpu
            self._project(
                n_iter,
                U=op,
                U_dagger=op_dag,
