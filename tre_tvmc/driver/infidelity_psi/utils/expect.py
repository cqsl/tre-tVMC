...
def _expect_2distr(
    ...
    log_L_σ, aux = nkjax.apply_chunked(
        partial(log_expected_fun, return_aux=True),
        in_axes=(None, None, 0, 0, *args_in_axes),
        chunk_size=chunk_size,
    )(pars_new, pars_old, σ_new, σ_old, *args)
...
def _expect_bwd_fid(
    ...
    def f(pars_new, pars_old, σ_new, σ_old, ΔL_σ, *all_args):
        ...
        term2 = jnp.exp(
            log_term2 - log_N
        )  # only that term needs recorrecting, the other has been corrected
...
def compute_weighted_stats(log_Llocs, log_w, log_N):
    ...
    cov_F_wc = (
        mpi_statistics(Llocs * wc).mean - Lstats.mean * wc_stats.mean
    )  # correct for mean?

    new_var_F = (
        Lstats.variance
        + (Lstats.mean**2) * wc_stats.variance
        - 2 * Lstats.mean * cov_F_wc
    )
    ...
    Lstats = Lstats.replace(
        variance=new_var_F, error_of_mean=jnp.sqrt(new_var_F / n_total)
    )
    return Lstats, Llocs
