...
def validate_arguments(
    vstate: MCState, op: InfidelityOperatorUPsi, chunk_size: int = None
):
...
def get_local_kernel_arguments_infidelity_with_in_axes(
    vstate: MCState,
    target: MCState,
    O: ContinuousOperator,
...
def _local_kernel_log_fidelity_jax(  # NO weights
    ...
    if cv_coeff is not None:
        log_res = logsumexp(
            jnp.stack(
                [log_res, 2 * log_Floc_xy.real, jnp.zeros_like(log_res)], axis=-1
            ),
...
def infidelity_chunked(vstate: MCState, op: InfidelityOperatorUPsi, chunk_size: int):
    L_stats, wc = _infidelity_and_grad_chunked(
        vstate, op, chunk_size, return_grad=False
    )
    op._wc = wc
    return L_stats
...
def infidelity_and_forces_chunked(  # noqa: F811
    ...
    *,
    ...
    L_stats, L_grad, wc = _infidelity_and_grad_chunked(
        vstate, op, chunk_size, return_grad=True
    )
    op._wc = wc
    return L_stats, L_grad


def _infidelity_and_grad_chunked(
    vstate: MCState,
    op: InfidelityOperatorUPsi,
    chunk_size: int,
    ...
    (
        ...
        args,
        args_in_axes,
    ) = get_local_kernel_arguments_infidelity_with_in_axes(
        vstate, op.target, op._U, op._U_dagger
    )

    # functions to compute Floc etc
    local_log_infidelity_kernel = nk.utils.HashablePartial(
        _local_kernel_log_fidelity_jax, op._U, op._U_dagger
    )
    local_log_weights_kernel = nk.utils.HashablePartial(
        _local_kernel_log_weights_jax, op._U, op._U_dagger, include_sqrt=op._sample_sqrt
    )
...
def local_log_fidelity_kernel_wrapper(  # add some spice
    ...
    log_res = local_log_fidelity_kernel(
        afun, afun_t, W, W_t, σ, σ_t, *args, cv_coeff=cv_coeff
    )
...
def infidelity_sampling_MCState(
    ...
    def expect_kernel(params):
        ...
        log_weights_fn = (
            lambda params, params_t, σ, σ_t, *args: local_log_weights_kernel_wrapper(
                local_weights_fun,
                afun,
                params,
                σ,
